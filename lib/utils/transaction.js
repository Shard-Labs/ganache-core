...
module.exports = class Transaction extends EthereumJsTransaction {
  ...
  static fromJSON(json, type, common, networkId, hardfork) {
    ...
    /**
     * Fee currency was added so that it can be sent to Transaction object.
     * Gateway fee and gateway fee recipient were not added because they
     * are only used in production.
     */
    const options = {
      ...
      feeCurrency: ethUtil.toBuffer(to.hex(json.feeCurrency)),
  ...
  encode() {
    const resultJSON = {
      ...
      feeCurrency: to.nullableRpcQuantityHexString(this.feeCurrency),
