...
const { RegistryProxy } = require("./predeployedContracts.js");
...
const defaultOptions = {
  ...
  useExperimentalOpcodes: false // Allow the use of additional Istanbul opcodes for EIP 1884 (BALANCE EXTCODEHASH SLOAD)
};
...
BlockchainDouble.prototype.deployContract = function(contract, callback) {
  const code = utils.toBuffer(contract.code);
  const storageKey = utils.toBuffer(contract.storageKey);
  const storageValue = utils.toBuffer(contract.storageValue);

  async.series(
    [
      (next) => this.vm.stateManager.putContractCode(utils.toBuffer(contract.address), code, next),
      (next) => this.vm.stateManager._cache.flush(next),
      (next) =>
        this.vm.stateManager.putContractStorage(utils.toBuffer(contract.address), storageKey, storageValue, next),
      (next) => this.vm.stateManager._cache.flush(next)
    ],
    callback
  );
