...
function fixProps(tx, data) {
  ...
  // and `null`/`undefined`.
  tx._originals = [];
  const fieldNames = ["nonce", "gasPrice", "gasLimit", "value", "feeCurrency"];
...
module.exports = class Transaction extends EthereumJsTransaction {
  ...
  static fromJSON(json, type, common, networkId, hardfork) {
    ...
    if (json.feeCurrency) {
      options.feeCurrency = to.buffer(json.feeCurrency);
    }
  ...
  encode() {
    const resultJSON = {
      ...
      feeCurrency: to.nullableRpcDataHexString(this.feeCurrency),
      _type: this.type,
      _options: {
