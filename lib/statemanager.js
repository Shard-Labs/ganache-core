...
var ECIES = require("@celo/utils").ECIES;
...
function StateManager(options, provider) {
  ...
  this.gasPriceFeeCurrencyRatio = options.gasPriceFeeCurrencyRatio;
...
StateManager.prototype.gasPrice = function(address) {
  if (address) {
    return to.rpcQuantityHexString(this.gasPriceFeeCurrencyRatio * to.number(this.gasPriceVal));
  } else {
...
StateManager.prototype._setTransactionDefaults = function(tx, isTransaction) {
  ...
  if (tx.gasPrice.length === 0) {
    tx.gasPrice = this.gasPrice(tx.feeCurrency);
  }
...
StateManager.prototype.decrypt = function(address, dataToEncrypt) {
  ...
  const ciphertext = to.buffer(dataToEncrypt, "hex");
  const pKey = Buffer.from(account.secretKey);

  return "0x" + ECIES.Decrypt(pKey, ciphertext).toString("hex");
};
...
// see https://web3js.readthedocs.io/en/v1.2.7/web3-eth.html#signtransaction
StateManager.prototype.signTx = function(txData, address) {
  const account = this.accounts[to.hex(address).toLowerCase()];
  if (!account) {
    throw new Error("cannot sign tx; no private key which matches 'from'");
  }

  const tx = new Transaction(txData);
  tx.sign(Buffer.from(account.secretKey));
  return {
